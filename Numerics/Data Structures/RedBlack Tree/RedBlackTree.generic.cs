...
namespace Orbifold.Numerics
{
	...
	public class RedBlackTree<T> : BinarySearchTreeBase<T>
	{
		...
		protected override void AddItem(T item)
		{
			...
			var root = this.Tree as RedBlackTreeNode<T>;
			var newRoot = this.InsertNode(root, item);
		...
		protected override bool RemoveItem(T item)
		{
			if(this.Tree != null) {
				...
				startNode.Right = (RedBlackTreeNode<T>)this.Tree;
				...
				while(childNode[direction] != null) {
					...
					var comparisonValue = this.Comparer.Compare(childNode.Data, item);
				...
				this.Tree = startNode.Right;

				if(this.Tree != null) {
					((RedBlackTreeNode<T>)this.Tree).Color = NodeColor.Black;
				}
		...
		private RedBlackTreeNode<T> InsertNode(RedBlackTreeNode<T> node, T item)
		{
			if(node == null)
			...
			else if(this.Comparer.Compare(item, node.Data) != 0) {
				var direction = this.Comparer.Compare(node.Data, item) < 0;
				node[direction] = this.InsertNode(node[direction], item);
				...
				throw new ArgumentException(Resource.IsAlreadyInTheStructure);
			return node;
		}
	}
}